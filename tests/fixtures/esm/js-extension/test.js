...
function implicitEsm() {
  try {
    cli.command({
      ...
      handler: ({ config }) => {
        if (config && Object.keys(config).length > 0) {
          log.verbose(log.chalk.green('esm:js-extension'), log.chalk.green('success'))
        } else {
    ...
    log.error(log.chalk.green('esm:js-extension'), log.chalk.gray(err.message.replaceAll(os.EOL, ' ')))
