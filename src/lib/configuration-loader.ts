 ...
 */
async function withEsbuild(filePath: string, pkgInfo: PackageInfo) {
 ...
 * results.
 */
async function withBabelRegister(filePath: string, pkgInfo: PackageInfo) {
  try {
    if (!pkgInfo.root) throw new Error('Unable to determine host package root directory.');
...
export default async function configurationLoader(filePath: string, content: string) {
   ...
   */
  try {
    const config = await withEsbuild(filePath, pkgInfo);
   ...
   */
  try {
    const config = await withBabelRegister(filePath, pkgInfo);
